...
```bash
HugoLockAssistant.exe --method=<method> --mode=<mode> [--extracmd=<额外参数>] [--hide]
```
...
### HugoLaunchTool 工具 - 直接模式

```cmd
HugoLockAssistant.exe --method=launchtool --mode=direct
```

### HugoLock 工具 - 直接模式 不显示窗口

```cmd
HugoLockAssistant.exe --method=lock --mode=direct --hide
```
