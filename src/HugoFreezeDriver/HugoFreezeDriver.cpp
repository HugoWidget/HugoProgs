...
void PrintDriverStatus(const QueryResult& result)
{
	...
	if (result.bootConfig.querySuccess) {
		wcout << HFreezeDriver::HexDump(result.bootConfig.buffer, result.bootConfig.validLen);
	}
	else {
...
// 查询并打印驱动状态
void QueryAndPrint(HFreezeDriver& driver)
{
	QueryResult result = driver.QueryDriverStatus();
	FreezeResult stateResult = driver.ParseFreezeBuffer(result);
	std::wcout << HFreezeDriver::FormatFreezeStateResult(
		stateResult,
		result.runtimeStatus.querySuccess ? result.runtimeStatus.activeFlag : 0,
		result.runtimeStatus.querySuccess ? result.runtimeStatus.ptr1 : 0,
...
// 应用冻结设置（盘符列表）
void ApplyFreezeSettings(HFreezeDriver& driver, const wstring& disks)
{
	wstring input = disks;
	if (disks == L"0")input = L"";
	...
	auto result = driver.SetFreezeState(input);
